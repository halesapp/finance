...
export function BankList() {
  ...
  async function handleSave(bank) {
    if (editing === 'new') {
      await insert(bank)
    } else {
      await update(editing.id, bank)
    }
    setEditing(null)
  }
  ...
  return (
    <div class="space-y-3">
      ...
      {banks.length === 0 ? (
        <p class="text-gray-500 dark:text-gray-400 text-sm">No banks yet.</p>
      ) : (
